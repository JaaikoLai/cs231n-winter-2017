...
class CaptioningRNN(object):
    ...
    def loss(self, features, captions):
        ...
        yout, cache_yout = temporal_affine_forward(h, W_vocab, b_vocab)
        loss, dyout = temporal_softmax_loss(yout, captions_out, mask)
        
        dh, dW_vocab, db_vocab = temporal_affine_backward(dyout, cache_yout)
        if self.cell_type == 'rnn':
        ...
        '''
        grads['W_embed'] = self.params['W_embed']
        # Initialize CNN -> hidden state projection parameters
        grads['W_proj'] = self.params['W_proj']
        grads['b_proj'] = self.params['b_proj']

        # Initialize parameters for the RNN
        grads['Wx'] = self.params['Wx']
        grads['Wh'] = self.params['Wh']
        grads['b'] = self.params['b']

        # Initialize output to vocab weights
        grads['W_vocab'] = self.params['W_vocab']
    ...
    def sample(self, features, max_length=30):
        ...
        yout, cache_yout = temporal_affine_forward(h, W_vocab, b_vocab)
        captions = np.argsort(-yout,axis=2)[:,:,0]
